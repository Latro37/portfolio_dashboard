  ...
  return (
    <Card className="border-border/50">
      <CardContent className="p-4">
        ...
        <p className={`mt-1 text-3xl font-semibold truncate ${color || "text-foreground"}`}>{value}</p>
      </CardContent>
    </Card>
  );
}
...
export function MetricCards({ summary }: Props) {
  ...
  return (
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
      ...
      <Metric label="Win Rate" value={parseFloat(s.win_rate.toFixed(2)) + "%"} />
      <Metric label="Sortino" value={String(parseFloat(s.sortino_ratio.toFixed(2)))} />
      <Metric label="Volatility" value={parseFloat(s.annualized_volatility.toFixed(2)) + "%"} />
      ...
      <Metric label="Calmar" value={String(parseFloat(s.calmar_ratio.toFixed(2)))} />
