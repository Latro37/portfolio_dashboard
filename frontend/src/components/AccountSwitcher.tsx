...
export function AccountSwitcher({
  ...
  const isGlobalAll = selectedCredential === "__all__";
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      {/* Sub-account selector — hidden when global "All Accounts" is selected */}
      {!isGlobalAll && (
        <div className="relative">
          <select
            ...
            className="appearance-none rounded-md border border-border bg-muted px-3 py-1.5 pr-8 text-sm font-medium text-foreground focus:outline-none focus:ring-1 focus:ring-ring"
          >
            {subAccounts.length > 1 && <option value="all">All Sub-Accounts</option>}
            {subAccounts.map((a) => (
